...
class FilmorateApplicationTests {
    ...
    void shouldCreateNewFilm() {
        ...
        ResponseEntity<Film> entity4 = template.postForEntity("/films", notValidDuration,
                Film.class);
        assertEquals(HttpStatus.INTERNAL_SERVER_ERROR, entity4.getStatusCode());
    }
    ...
    void shouldCreateNewUser() {
        ...
        User notValidLogin = new User(1, "mail@mail.ru","dolore ullamco","Nick Name",
                LocalDate.parse("1946-08-20"));
        ResponseEntity<Film> entity1 = template.postForEntity("/films", notValidLogin,
                Film.class);
        assertEquals(HttpStatus.INTERNAL_SERVER_ERROR, entity1.getStatusCode());

        User notValidEmail = new User(1, "mail&mail.ru","dolore","Nick Name",
                LocalDate.parse("1946-08-20"));
        ResponseEntity<Film> entity2 = template.postForEntity("/films", notValidEmail,
                Film.class);
        assertEquals(HttpStatus.INTERNAL_SERVER_ERROR, entity2.getStatusCode());

        User notValidBirthday = new User(1, "mail@mail.ru","dolore","Nick Name",
                LocalDate.parse("2030-08-20"));
        ResponseEntity<Film> entity3 = template.postForEntity("/films", notValidBirthday,
                Film.class);
        assertEquals(HttpStatus.INTERNAL_SERVER_ERROR, entity3.getStatusCode());
